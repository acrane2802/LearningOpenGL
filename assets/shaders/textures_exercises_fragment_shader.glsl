...
void main()
{
    ...
    // this mixes the two textures, the third argument mixes it based on the float imported from the vertex shader
    FragColor = mix(texture(textureColor1, texCoord), texture(textureColor2, vec2((texCoord.x * -1.0), texCoord.y)), outMix);
}